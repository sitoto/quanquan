...
class RatesControllerTest < ActionController::TestCase
  ...
  test "should create rate" do
    assert_difference('Rate.count') do
      post :create, rate: { from_url: @rate.from_url, name: @rate.name, position: @rate.position, rating: @rate.rating }
    end
  ...
  test "should update rate" do
    put :update, id: @rate, rate: { from_url: @rate.from_url, name: @rate.name, position: @rate.position, rating: @rate.rating }
    assert_redirected_to rate_path(assigns(:rate))
  end
